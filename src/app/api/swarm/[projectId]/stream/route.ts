...
export async function GET(
  ...
  // Pub/sub channel must use channelKey() — keyPrefix does not apply to pub/sub
  const channel = channelKey(`swarm:${projectId}`)

  const encoder = new TextEncoder()
  const stream = new ReadableStream({
    async start(controller) {
      // Dedicated subscriber connection — ioredis cannot share one connection for pub/sub
